...
describe("useResizablePanels", () => {
  ...
  it("reads stored sizes and clamps to bounds", () => {
    window.localStorage.setItem("micodemonitor.sidebarWidth", "999");
    window.localStorage.setItem("micodemonitor.rightPanelWidth", "100");
    window.localStorage.setItem("micodemonitor.planPanelHeight", "not-a-number");
  ...
  it("persists sidebar width changes and clamps max", () => {
    ...
    expect(hook.result.sidebarWidth).toBe(420);
    expect(window.localStorage.getItem("micodemonitor.sidebarWidth")).toBe(
      "420",
    );
