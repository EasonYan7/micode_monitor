...
pub(crate) async fn open_workspace_in(
    ...
    let status = if let Some(command) = command {
        ...
        #[cfg(target_os = "windows")]
        {
            // Resolve the editor command (handles display names like "Visual Studio Code",
            // finds full paths, and produces actionable error messages).
            // Always invoke via `cmd /C` so that .cmd/.bat wrappers on PATH are executed
            // correctly by the Windows shell.
